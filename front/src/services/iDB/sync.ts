...
class Sync {
  ...
  // updateOnline = async ({ models, force } = {}) => {
  updateOnline = async (param: { models?: any, force?: boolean } = {}) => {
    const { socket } = this
    // if(!this.updateOnline.occupied){
    //   this.updateOnline.occupied = true
    if(!this.buzy){
      ...
      const data = await DB.getAllNews()
      console.log('data to send', data.length, data)
      if (socket && socket.connected && (param.force || data.length)) {
        socket.emit('updates', {
          data,
          models: param.models,
          from: LocalData.from,
